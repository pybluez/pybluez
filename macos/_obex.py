...
from ._IOBluetooth import OBEXSession, IOBluetoothDevice, IOBluetoothRFCOMMChannel
from ._LightAquaBlue import (
    BBBluetoothOBEXClient,
    BBBluetoothOBEXServer,
    BBStreamingInputStream,
    BBStreamingOutputStream,
...
_obexerrorcodes = {
    ...
    -21850: "general error",
    -21851: "no resources",
    -21852: "operation not supported",
    -21853: "internal error",
    -21854: "bad argument",
    -21855: "timeout",
    -21856: "bad request",
    -21857: "cancelled",
    -21875: "session is busy",
    -21876: "OBEX session not connected",
    -21877: "bad request in OBEX session",
    -21878: "bad response from other party",
    -21879: "Bluetooth transport not available",
    -21880: "Bluetooth transport connection died",
    -21881: "OBEX session timed out",
    -21882: "OBEX session already connected",
}
    ...
    for header, value in list(headers.items()):
        ...
        if mask == _HEADER_UNICODE:
            if not isinstance(value, str):
                raise TypeError(
                    "value for '%s' must be string, was %s" % (str(header), type(value))
                )
            ...
            try:
                ...
                raise TypeError(
                    "value for '%s' must be string, array or other buffer type, was %s"
            ...
            if not isinstance(value, int):
                raise TypeError(
                    "value for '%s' must be int, was %s" % (str(header), type(value))
                )
...
class OBEXClient(object):
    ...
    def __init__(self, address, channel):
        if not _lightbluecommon._isbtaddr(address):
            raise TypeError("address '%s' is not a valid bluetooth address" % address)
    ...
    def connect(self, headers={}):
        if self.__client is None:
            if not BBLocalDevice.isPoweredOn():
                raise OBEXError(
                    _kOBEXSessionNoTransportError, "Bluetooth device not available"
                )
        ...
        if r != _kOBEXSuccess:
            ...
            raise OBEXError(r, "error starting Connect request (%s)" % errdesc(r))
    ...
    def disconnect(self, headers={}):
        ...
        try:
            ...
            if self.__error != _kOBEXSuccess:
                raise OBEXError(
                    self.__error,
                    "error during Disconnect request (%s)" % errdesc(self.__error),
                )
        finally:
    ...
    def put(self, headers, fileobj):
        ...
        self.__fileobjdelegate = _macutil.BBFileLikeObjectReader.alloc().initWithFileLikeObject_(
            fileobj
        )
        self.instream = BBStreamingInputStream.alloc().initWithDelegate_(
            self.__fileobjdelegate
        )
        self.instream.open()
        ...
        if self.__error != _kOBEXSuccess:
            raise OBEXError(
                self.__error, "error during Put request (%s)" % errdesc(self.__error)
            )
        return self.__getresponse()
    ...
    def delete(self, headers):
        ...
        self.__reset()
        headerset = _headersdicttoset(headers)
        r = self.__client.sendPutRequestWithHeaders_readFromStream_(headerset, None)
        if r != _kOBEXSuccess:
            raise OBEXError(r, "error starting Delete request (%s)" % errdesc(r))

        _macutil.waituntil(self._done)
        if self.__error != _kOBEXSuccess:
            raise OBEXError(
                self.__error, "error during Delete request (%s)" % errdesc(self.__error)
            )
        return self.__getresponse()
    ...
    def get(self, headers, fileobj):
        ...
        self.__reset()
        headerset = _headersdicttoset(headers)
        delegate = _macutil.BBFileLikeObjectWriter.alloc().initWithFileLikeObject_(
            fileobj
        )
        outstream = BBStreamingOutputStream.alloc().initWithDelegate_(delegate)
        outstream.open()
        r = self.__client.sendGetRequestWithHeaders_writeToStream_(headerset, outstream)
        if r != _kOBEXSuccess:
        ...
        if self.__error != _kOBEXSuccess:
            raise OBEXError(
                self.__error, "error during Get request (%s)" % errdesc(self.__error)
            )
        return self.__getresponse()
    ...
    def setpath(self, headers, cdtoparent=False, createdirs=False):
        ...
        self.__reset()
        headerset = _headersdicttoset(headers)
        r = self.__client.sendSetPathRequestWithHeaders_changeToParentDirectoryFirst_createDirectoriesIfNeeded_(
            headerset, cdtoparent, createdirs
        )
        if r != _kOBEXSuccess:
            raise OBEXError(r, "error starting SetPath request (%s)" % errdesc(r))

        _macutil.waituntil(self._done)
        if self.__error != _kOBEXSuccess:
            raise OBEXError(
                self.__error,
                "error during SetPath request (%s)" % errdesc(self.__error),
            )
        return self.__getresponse()
    ...
    def _finishedrequest(self, error, response):
        if error in (_kOBEXSessionNoTransportError, _kOBEXSessionTransportDiedError):
            self.__closetransport()
        self.__error = error
    ...
    def __checkconnected(self):
        if self.__client is None:
            raise OBEXError(
                _kOBEXSessionNotConnectedError,
...
class _BBOBEXClientDelegate(NSObject):
    ...
    def client_didFinishConnectRequestWithError_response_(
        self, client, error, response
    ):
    ...
    #       response:(BBOBEXResponse *)response;
    def client_didFinishDisconnectRequestWithError_response_(
        self, client, error, response
    ):
        self._cb_requestdone(error, response)
    ...
    #       response:(BBOBEXResponse *)response;
    def client_didFinishPutRequestForStream_error_response_(
        self, client, instream, error, response
    ):
        self._cb_requestdone(error, response)
    ...
    #         error:(OBEXError)error
    #       response:(BBOBEXResponse *)response;
    def client_didFinishGetRequestForStream_error_response_(
        self, client, outstream, error, response
    ):
        self._cb_requestdone(error, response)
    ...
    #       response:(BBOBEXResponse *)response;
    def client_didFinishSetPathRequestWithError_response_(
        self, client, error, response
    ):
        self._cb_requestdone(error, response)
...
def sendfile(address, channel, source):
    ...
    if not isinstance(channel, int):
        raise TypeError("channel must be int, was %s" % type(channel))
    if not isinstance(source, str) and not hasattr(source, "read"):
...
class BBOBEXObjectPushServer(NSObject):
    def initWithChannel_fileLikeObject_(self, channel, fileobject):
        if not isinstance(channel, IOBluetoothRFCOMMChannel) and not isinstance(
            channel, OBEXSession
        ):
            raise TypeError(
                "internal error, channel is of wrong type %s" % type(channel)
            )
        ...
        self.__server = BBBluetoothOBEXServer.alloc().initWithIncomingRFCOMMChannel_delegate_(
            channel, self
        )
        ...
        if isinstance(channel, OBEXSession):
            self.__server.performSelector_withObject_("setOBEXSession:", channel)
        return self
    ...
    def server_shouldHandlePutRequest_(self, server, requestheaders):
        ...
        self.delegate = _macutil.BBFileLikeObjectWriter.alloc().initWithFileLikeObject_(
            self.__fileobject
        )
    ...
    server_didHandlePutRequestForStream_requestWasAborted_ = objc.selector(
        server_didHandlePutRequestForStream_requestWasAborted_, signature=b"v@:@@c"
    )
