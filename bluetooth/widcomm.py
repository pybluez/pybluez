...
class WCInquirer:
    ...
    def read_msg(self):
        ...
        if msg_type == _widcomm.INQ_DEVICE_RESPONDED:
            ...
            self.recently_discovered.append((bdaddr, devclass, bdname, connected))
    ...
    def start_discovery(self, addr, uuid=None):
        ...
        if uuid is not None:
            self.sdp_query_in_progress = self._wcinq.start_discovery(
                bd_addr, to_full_uuid(uuid)
            )
        else:
            self.sdp_query_in_progress = self._wcinq.start_discovery(bd_addr)
    ...
    def read_discovery_records(self, addr, uuid=None):
        ...
        if uuid is not None:
            ...
            return self._wcinq.read_discovery_records(bd_addr, to_full_uuid(uuid))
        else:
...
def discover_devices(
    duration=8, flush_cache=True, lookup_names=False, lookup_class=False
):
...
def advertise_service(
    ...
    sock._advertise_service(
        name, service_id, service_classes, profiles, provider, description, protocols
    )
...
def _port_return_code_to_str(code):
    k = {
        _widcomm.RFCOMM_SUCCESS: "Success",
        ...
        _widcomm.RFCOMM_PEER_CONNECTION_FAILED: "Connection ended by remote " "side",
...
def _port_ev_code_to_str(code):
    d = {
        _widcomm.PORT_EV_RXFLAG: "Received certain character",
...
class BluetoothSocket:
    ...
    def __rfcomm_advertise_service(
        self,
        name,
        service_id,
        service_classes,
        ...
        if protocols:
            raise NotImplementedError(
                "extra protocols not yet supported in Widcomm stack"
            )
        ...
        if profiles:
            for uuid, version in profiles:
                ...
                _sdp_checkraise(
                    self._sdpservice.add_profile_descriptor_list(uuid, version)
                )
    ...
    def l2cap_read_msg(self):
        ...
        if msg_type == _widcomm.L2CAP_DATA_RECEIVED:
            ...
            if not result:
                raise BluetoothError("accept() failed")
