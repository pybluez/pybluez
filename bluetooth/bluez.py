...
def write_flush_timeout(addr, timeout):
    ...
    response = _bt.hci_send_req(
        hci_sock, _bt.OGF_HOST_CTL, 0x0028, _bt.EVT_CMD_COMPLETE, 3, pkt
    )
...
def read_flush_timeout(addr):
    ...
    response = _bt.hci_send_req(
        hci_sock, _bt.OGF_HOST_CTL, 0x0027, _bt.EVT_CMD_COMPLETE, 5, pkt
    )
    status = get_byte(response[0])
    rhandle = struct.unpack("H", response[1:3])[0]
    assert rhandle == handle
...
class DeviceDiscoverer:
    ...
    def find_devices(self, lookup_names=True, duration=8, flush_cache=True):
        ...
        try:
            _bt.hci_send_cmd(self.sock, _bt.OGF_LINK_CTL, _bt.OCF_INQUIRY, cmd_pkt)
        except _bt.error as e:
            raise BluetoothError(*e.args)
    ...
    def cancel_inquiry(self):
        ...
        if self.is_inquiring:
            try:
                ...
                self.sock.close()
                self.sock = None
                raise BluetoothError(e.args[0], "error canceling inquiry: " + e.args[1])
            self.is_inquiring = False
    ...
    def _process_hci_event(self):
        ...
        if event == _bt.EVT_INQUIRY_RESULT:
            ...
            for i in range(nrsp):
                ...
                devclass = (
                    (devclass_raw[2] << 16) | (devclass_raw[1] << 8) | devclass_raw[0]
                )
            ...
            _bt.HAVE_EVT_EXTENDED_INQUIRY_RESULT
            and event == _bt.EVT_EXTENDED_INQUIRY_RESULT
        ):
            nrsp = get_byte(pkt[0])
            for i in range(nrsp):
                addr = _bt.ba2str(pkt[1 + 6 * i : 1 + 6 * i + 6])
                ...
                data_len = (
                    _bt.EXTENDED_INQUIRY_INFO_SIZE - _bt.INQUIRY_INFO_WITH_RSSI_SIZE
                )
    ...
    def _device_discovered(
        self, address, device_class, psrm, pspm, clockoff, rssi, name
    ):
        if self.lookup_names:
            if name is not None:
                self.device_discovered(address, device_class, rssi, name)
            elif address not in self.names_found and address not in self.names_to_find:

                self.names_to_find[address] = (device_class, rssi, psrm, pspm, clockoff)
        else:
    ...
    def _send_next_name_req(self):
        ...
        try:
            _bt.hci_send_cmd(
                self.sock, _bt.OGF_LINK_CTL, _bt.OCF_REMOTE_NAME_REQ, cmd_pkt
            )
        except _bt.error as e:
            raise BluetoothError(
                e.args[0], "error request name of %s - %s:" % (address, e.args[1])
            )
    ...
    def device_discovered(self, address, device_class, rssi, name):
        ...
        if name:
            ...
            print(("found: %s (class 0x%X, rssi %s)" % (address, device_class, rssi)))
