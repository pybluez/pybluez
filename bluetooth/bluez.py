...
from bluetooth._bluetooth import L2CAP, L2CAP_OPTIONS, RFCOMM, SOL_L2CAP
from bluetooth.btcommon import (IAC_GIAC, BluetoothError, is_valid_address,
                                is_valid_uuid)
...
def discover_devices(duration=8, flush_cache=True, lookup_names=False,
    ...
    try:
        results = _bt.hci_inquiry(
            sock, duration=duration, flush_cache=flush_cache,
            lookup_class=lookup_class, device_id=device_id, iac=iac)
    except _bt.error as e:
        sock.close()
        raise BluetoothError(
            e.args[0],
            "Error communicating with local bluetooth adapter: " + e.args[1])
    ...
    if lookup_names:
        ...
        for item in results:
            ...
            try:
                ...
                continue
            pairs.append(
                (addr, name, dev_class) if lookup_class else (addr, name))
        sock.close()
        return pairs
    else:
        sock.close()
...
def read_local_bdaddr() -> list[str]:
    try:
        hci_sock = _bt.hci_open_dev(0)
        old_filter = hci_sock.getsockopt(_bt.SOL_HCI, _bt.HCI_FILTER, 14)
        flt = _bt.hci_filter_new()
        opcode = _bt.cmd_opcode_pack(
            _bt.OGF_INFO_PARAM,
...
def lookup_name(address: str, timeout: float = 10.0):
    if not is_valid_address(address):
        raise BluetoothError(
            EINVAL, "%s is not a valid Bluetooth address" % address)

    sock: socket = _gethcisock()
    timeoutms: int = int(timeout * 1000)
    try:
...
def set_packet_timeout(address: str, timeout: float):
    """
    ...
    """
    write_flush_timeout(address, round(timeout / 0.625))


def get_l2cap_options(sock: socket) -> list:
...
def set_l2cap_options(sock: socket, options: list) -> None:
...
def set_l2cap_mtu(sock: socket, mtu: int):
...
class BluetoothSocket:
    ...
    def __init__(self, proto=RFCOMM, _sock: socket = None):
        if _sock is None:
            _sock = _bt.btsocket(proto)
        self._sock: socket = _sock
        self._proto = proto

    def dup(self) -> BluetoothSocket:
        """dup () -> socket object
    ...
    def get_l2cap_options(self) -> list:
        """get_l2cap_options (sock)
    ...
    def set_l2cap_options(self, options) -> None:
        """set_l2cap_options (sock, options)
    ...
    def set_l2cap_mtu(self, mtu):
        ...
        Adjusts the MTU for the specified L2CAP socket.
        This method needs to be invoked on both sides of the connection
        for it to work! The default mtu that all L2CAP connections
        start with is 672 bytes.
...
def advertise_service(sock, name, service_id: str = "",
    ...
    try:
        _bt.sdp_advertise_service(
            sock._sock, name, service_id, service_classes,
            profiles, provider, description, protocols)
    except _bt.error as e:
        raise BluetoothError(*e.args)


def stop_advertising(sock: socket) -> None:
    try:
...
def _gethcisock(device_id: int = -1) -> socket:
    try:
        sock: socket = _bt.hci_open_dev(device_id)
    except _bt.error as e:
        raise BluetoothError(
            e.args[0], "error accessing bluetooth device: " + e.args[1])
    return sock


def get_acl_conn_handle(hci_sock: socket, addr: str):
    ...
    return struct.unpack("8xH14x", request.tostring())[0]


def write_flush_timeout(addr: str, timeout: float):
    ...
    response = _bt.hci_send_req(
        hci_sock, _bt.OGF_HOST_CTL,
...
def read_flush_timeout(addr: str):
    hci_sock = _bt.hci_open_dev()
    # get the ACL connection handle to the remote device
    handle = get_acl_conn_handle(hci_sock, addr)
...
def byte_to_signed_int(byteval) -> int:
    return byteval - 256 if byteval > 127 else byteval
...
class DeviceDiscoverer:
    ...
    def __init__(self, device_id: int = -1):
        ...
        self.is_inquiring: bool = False
        self.lookup_names: bool = False
        self.device_id: int = device_id

        self.names_to_find = {}
        self.names_found = {}

    def find_devices(self, lookup_names: bool = True,
                     duration: int = 8,
                     flush_cache: bool = True) -> None:
        """
    ...
    def cancel_inquiry(self) -> None:
        ...
        if self.is_inquiring:
            try:
                _bt.hci_send_cmd(
                    self.sock, _bt.OGF_LINK_CTL, _bt.OCF_INQUIRY_CANCEL)
            except _bt.error as e:
                self.sock.close()
                self.sock = None
                raise BluetoothError(
                    e.args[0], "error canceling inquiry: " + e.args[1])
            self.is_inquiring = False

    def process_inquiry(self) -> None:
        """
    ...
    def _process_hci_event(self) -> None:
        ...
        if event == _bt.EVT_INQUIRY_RESULT:
            nrsp = get_byte(pkt[0])
            for i in range(nrsp):
                # TODO: Unravel and simplify these literals
                ...
                devclass = ((devclass_raw[2] << 16)
                            ...
                            | devclass_raw[0])
                clockoff = \
                    pkt[(1 + 12 * nrsp + 2 * i):(1 + 12 * nrsp + 2 * i + 2)]

                self._device_discovered(
                    addr, devclass, psrm, pspm, clockoff, None, None)
            ...
            for i in range(nrsp):
                ...
                devclass = ((devclass_raw[2] << 16)
                            ...
                            | devclass_raw[0])
                clockoff = \
                    pkt[(1 + 11 * nrsp + 2 * i):(1 + 11 * nrsp + 2 * i + 2)]
                rssi = byte_to_signed_int(get_byte(pkt[1 + 13 * nrsp + i]))

                self._device_discovered(
                    addr, devclass, psrm, pspm, clockoff, rssi, None)
        elif (_bt.HAVE_EVT_EXTENDED_INQUIRY_RESULT and
              event == _bt.EVT_EXTENDED_INQUIRY_RESULT):
            nrsp = get_byte(pkt[0])
            for i in range(nrsp):
                addr = _bt.ba2str(pkt[(1 + 6 * i):(1 + 6 * i + 6)])
                psrm = pkt[1 + 6 * nrsp + i]
                ...
                self._device_discovered(
                    addr, devclass, psrm, pspm, clockoff, rssi, name)
        elif (event == _bt.EVT_INQUIRY_COMPLETE or
              event == _bt.EVT_CMD_COMPLETE):
            self.is_inquiring = False
            if not len(self.names_to_find):
                self.sock.close()
    ...
    def _device_discovered(self, address: str, device_class,
                           psrm, pspm, clockoff, rssi, name) -> None:
    ...
    def _send_next_name_req(self) -> None:
        assert len(self.names_to_find) > 0
        address: str = list(self.names_to_find.keys())[0]
        device_class, rssi, psrm, pspm, clockoff = self.names_to_find[address]
        # TODO not supported in python3
    ...
    def device_discovered(self, address: str, device_class,
                          rssi, name: str) -> None:
        """
