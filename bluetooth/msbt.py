...
class BluetoothSocket:
    def __init__(self, proto: int = RFCOMM, sockfd=None):
...
def advertise_service(sock: socket, name: str, service_id: str = "",
                      service_classes: list[str] = [],
                      profiles: list = [], provider: str = "",
                      description: str = "", protocols: list = []):
    ...
    if len(profiles) > 0:
        seq = [
            ...
            for uuid, version in profiles]
        avpairs.append((
            "UInt16", BLUETOOTH_PROFILE_DESCRIPTOR_LIST_ATTRID))
        avpairs.append(("ElemSeq", seq))
    ...
    sock._raw_sdp_record = \
        sdp_make_data_element("ElemSeq", avpairs)
...
def find_service(name: str = None, uuid: str = None,
    ...
    if address is not None:
        ...
        addresses: list[str] = discover_devices(lookup_names=False)
    ...
    for addr in addresses:
        ...
        for dict in dresults:
            ...
            dict["provider"] = record.get(PROVIDER_NAME_ATTRID)
            dict["service-id"] = record.get(SERVICE_ID_ATTRID)

            # XXX the C version is buggy (retrieves an extra byte or two),
            # so get the service name here even though it may have already
            # been set
            dict["name"] = record.get(SERVICE_NAME_ATTRID)
            dict["handle"] = record.get(SERVICE_RECORD_HANDLE_ATTRID)
