...
def to_full_uuid(uuid: str) -> str:
    """
    converts a short 16-bit or 32-bit reserved UUID to a full 128-bit
    Bluetooth UUID.
    """
...
def sdp_parse_size_desc(data) -> tuple[int, int, int]:
    dts = struct.unpack("B", data[0:1])[0]
    dtype, dsizedesc = dts >> 3, dts & 0x7
    dstart: int = 1
    if dtype == 0:
        dsize = 0
    elif dsizedesc >= 0 and dsizedesc < 5:
        dsize = 2 ** dsizedesc
    elif dsizedesc == 5:
...
def sdp_parse_uuid(data, size) -> str:
    if size in [2, 4]:
...
def sdp_parse_int(data, size: int, signed: bool) -> int:
    ...
    if fmt in ["!qq", "!QQ"]:
        ...
        result: int = (upp << 64) | low
        if signed:
            result = -((~ (result - 1)) & ~(-(1 << 127)))
        return result
...
def sdp_parse_data_element(data) -> tuple[str, any, int]:
...
def sdp_parse_raw_record(data):
    dtype, _, dstart = sdp_parse_size_desc(data)
    assert dtype == 6
...
def sdp_make_data_element(type: str, value) -> bytes:
